...
export const LoadingTable: FC<IProps> = ({ loading, error, count = 10, withCircle, padding }) => {
	if (!loading || (!error && !loading)) return null;

	return (
		<Flex width="100%" direction="column" padding={padding}>
			{loading && !error && (
				<ContentLoader width="100%" height={`${count * 60}px`}>
					{Array.from(Array(count).keys()).map((n: number) => (
						<Fragment key={n}>
							...
							<rect x={withCircle ? '100' : '0'} y={n === 0 ? 14 : n * 61 + 14} rx="5" ry="5" width="10%" height="18" />
							<rect
